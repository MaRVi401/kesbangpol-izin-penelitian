            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2 dark:text-gray-500">Verifikasi Mahasiswa</span>
                    </div>
                </li>
            </ol>
        </nav>
                                            ...
                                            <div class="relative bg-white rounded-xl shadow-lg dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
                                                <div class="flex items-center justify-between p-4 border-b dark:border-gray-700">
                                                    ...
                                                    <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="modal-{{ $user->uuid }}">
                                                        <svg class="w-3 h-3" fill="none" viewBox="0 0 14 14"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/></svg>
                                                    </button>
                                                </div>
                                                <div class="p-6 space-y-6">
                                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                        ...
                                                        <div class="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg border border-gray-100 dark:border-gray-600">
                                                            ...
                                                            <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ $user->mahasiswa->nim }}</p>
                                                        </div>

                                                        {{-- WhatsApp --}}
                                                        <div class="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg border border-gray-100 dark:border-gray-600">
                                                            <p class="text-xs text-gray-500 uppercase font-bold mb-1">WhatsApp</p>
                                                            <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ $user->no_wa ?? '-' }}</p>
                                                        </div>
                                                        ...
                                                        {{-- Alamat (Full Width) --}}
                                                        <div class="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg border border-gray-100 dark:border-gray-600 md:col-span-2">
                                                            <p class="text-xs text-gray-500 uppercase font-bold mb-1">Alamat Lengkap</p>
                                                            <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ $user->alamat ?? 'Alamat tidak diisi' }}</p>
                                                        </div>
                                                    </div>
                                                ...
                                                <div class="flex items-center justify-end p-6 space-x-3 border-t dark:border-gray-700">
                                                    {{-- ROUTE DIUBAH MENJADI operator.user-management.activate --}}
                                                    <form action="{{ route('operator.user-management.activate', $user->uuid) }}" method="POST">
                                                        @csrf
