...
@section('content')
    <div class="p-4 mt-14">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        <button type="button"
                                            ...
                                            class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-yellow-500 rounded-lg hover:bg-yellow-600 focus:ring-4 focus:outline-none focus:ring-yellow-300 dark:bg-yellow-600 dark:hover:bg-yellow-700 dark:focus:ring-yellow-800 transition-all shadow-sm"
                                            title="Edit Tiket">
        ...
        @foreach($tickets as $ticket)
            ...
            <div id="edit-modal-{{ $ticket->uuid }}" tabindex="-1" aria-hidden="true" 
                class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
                <div class="relative w-full max-w-md max-h-full">
                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
                        <div class="flex items-start justify-between p-4 border-b rounded-t dark:border-gray-600">
                            <h3 class="text-xl font-semibold text-gray-900 dark:text-white">
                                Edit Tiket: {{ $ticket->no_tiket }}
                            </h3>
                            <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="edit-modal-{{ $ticket->uuid }}">
                                <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
                                </svg>
                        ...
                        <form action="{{ route('ticket.update', $ticket->uuid) }}" method="POST">
                            ...
                            <input type="hidden" name="komentar" value="Revisi atribut surat via riwayat">

                            <div class="p-6 space-y-4">
                                <div>
                                    <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nomor Surat</label>
                                    <input type="text" name="nomor_surat" value="{{ $ticket->suratIzinPenelitian->nomor_surat ?? '' }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                                </div>
                                <div>
                                    <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Penandatangan</label>
                                    <select name="penandatangan_user_uuid" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                                        <option value="">-- Pilih Penandatangan --</option>
                                        @foreach($penandatangan_list as $p)
                                            @php
                                                $isSelected = false;
                                                if ($ticket->suratIzinPenelitian) {
                                                    $isKaban = $ticket->suratIzinPenelitian->penandatangan_type === 'App\Models\Kaban' && $ticket->suratIzinPenelitian->penandatangan_id === $p->kaban?->uuid;
                                                    $isKabid = $ticket->suratIzinPenelitian->penandatangan_type === 'App\Models\Kabid' && $ticket->suratIzinPenelitian->penandatangan_id === $p->kabid?->uuid;
                                                    $isSelected = $isKaban || $isKabid;
                                                }
                                            @endphp
                                            <option value="{{ $p->uuid }}" {{ $isSelected ? 'selected' : '' }}>
                                                {{ $p->nama }} - {{ strtoupper($p->role) }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="flex items-center p-6 space-x-2 border-t border-gray-200 rounded-b dark:border-gray-600">
                                ...
                                <button data-modal-hide="edit-modal-{{ $ticket->uuid }}" type="button" class="text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600">Batal</button>
                            </div>
                        </form>
                    </div>
                </div>
